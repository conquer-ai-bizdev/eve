...
```ts
{
  ...
  mode?: "foreground" | "background"; // background returns a reusable child session immediately
}
```
...
eve lowers every subagent into a model-visible tool. The parent packs `message`
with everything the child needs, since the child never sees the parent's
history. Set `outputSchema` to run the child in task mode, returning structured
output as the tool result. Built-in and declared local subagents also accept
`mode`: `"foreground"` is the default, while `"background"` releases the parent
after child creation and keeps the child as a reusable conversation for
authored supervision tools. [Remote agents](./guides/remote-agents) remain
foreground calls and expose `{ message, outputSchema? }`.
