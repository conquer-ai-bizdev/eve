...
An authenticated server-side operator route can run Bash against an agent's normal sandbox lifecycle without invoking the model:

```ts
import { runOperatorSandboxCommand } from "eve/operator";

const result = await runOperatorSandboxCommand({
...
The supplied session ID selects the same sandbox key as ordinary agent tools. The call creates or reattaches the sandbox, applies `bootstrap` and `onSession` when required, and preserves it after the command. This function does not authenticate the caller; expose it only behind an application-owned operator authorization boundary.
...
Sessions are persistent by default, and how the underlying runtime idles out depends on the backend. On the Vercel backend, the VM times out after a period of inactivity (default 30 minutes); eve preserves the filesystem and resumes the sandbox on the next message while the persisted sandbox remains available. Set `persistent: false` on `vercel()` when a stopped session sandbox should not create a persistence snapshot. The Docker backend keeps a long-lived container per durable session and persists `/workspace` across turns without that timeout, and the just-bash backend stores its virtual filesystem under `.eve/sandbox-cache/`.
...
eve records the exact provider sandbox and session-owned persistence snapshot
references as plaintext `$eve.*` attributes on the owning Workflow run. An
authenticated operator integration can read them with
`getOperatorWorkflowRunAttributes(runId)` from `eve/operator`. These references
are observability metadata: reusable template snapshots and author-provided
source snapshots are excluded, and an attribution-write failure does not change
the sandbox lifecycle result.
