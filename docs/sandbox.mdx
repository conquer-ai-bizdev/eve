...
Sandbox sessions retain their filesystem while the underlying sandbox remains available. How a backend retains state after compute stops depends on that backend. Vercel session sandboxes default to `persistent: true`, so Vercel preserves their filesystem for a later resume. Set `vercel({ persistent: false })` for disposable session sandboxes that do not need provider persistence after they stop. eve still creates non-persistent template sandboxes and explicitly snapshots them after bootstrap; this option changes fresh session sandboxes only. The Docker backend keeps a long-lived container per durable session, and the just-bash backend stores its virtual filesystem under `.eve/sandbox-cache/`.
...
When the eve server stops, no sandbox compute outlives it. `eve dev` stops the sandboxes it started when the dev server closes, and a self-hosted production server stops every open sandbox on shutdown (`SIGTERM`/`SIGINT`). Persistent backends can reattach a durable session from their stopped container, VM, or snapshot on the next server start. A non-persistent Vercel session does not retain its filesystem through that stop. Custom `SandboxBackend` adapters participate through the handle's `shutdown()` method: stop the underlying compute, keeping the session reattachable where the backend supports it.
...
- [Subagents](./subagents): built-in `agent` copies share their parent's sandbox; declared subagents have independent sandboxes.
