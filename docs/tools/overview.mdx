...
- `ctx.agent`: compiler-owned identity of the local agent node executing the tool, including its behavior revision.
- `ctx.getAgent(nodeId)`: look up another local node in the compiled agent graph by its stored id.
...
- `ctx.operationId`: a stable idempotency key for the logical tool invocation. It remains the same when Workflow replays a crashed turn even if the model provider generates a new `callId`.
